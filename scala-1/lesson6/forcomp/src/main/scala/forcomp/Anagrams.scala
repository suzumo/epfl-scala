...
//  val dictionary: List[Word] = List("art", "artificial", "i", "cat", "liar", "a", "larc", "fict")
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy((c: Char) =>
    c).mapValues(_.size).toList.sortWith((e1: (Char, Int), e2: (Char, Int)) => e1._1 < e2._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s.flatMap((w: Word) =>
    wordOccurrences(w)).groupBy(_._1).mapValues(_.map(_._2).sum).toList.sortWith((e1: (Char, Int), e2: (Char, Int)) => e1._1 < e2._1)
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.map((w: Word) =>
    (wordOccurrences(w), w)).groupBy(_._1).mapValues(_.map(_._2))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(List())
    case x::Nil => combinationsS(x)
    case x::xs  => {
      val xlist: List[Occurrences] = combinationsS(x)
      var xslist: List[Occurrences] = List.empty
      for (b <- combinations(xs); a <- xlist) xslist = xslist ++ List(a ++ b)
      xslist
    }
  }

  def combinationsS(tuple2: (Char, Int)): List[Occurrences] = tuple2 match {
    case (_, 0) => List(List())
    case (_, i) => combinationsS((tuple2._1, i-1)) ++ List(List((tuple2._1, i)))
  }
